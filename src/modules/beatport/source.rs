	...
	WSError: std::error::Error,
{
	...
	let item_track_id = item.track_id
		.map_err(
	...
	progress.item(
		id,
		&format!("{} [{}]", item_track_id, duration)
	);

	let item_track_id = id_cleaner.clean(&item_track_id);

	let similarity = sim::str(&item_track_id, track_id);
