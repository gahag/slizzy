	...
	SearchConfig: DeserializeOwned,
{
	fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
		...
		Ok(
			Config {
				sim_threshold: config.bandcamp.sim_threshold,
				search: config.bandcamp.search,
...
struct ConfigFile<SearchConfig> where SearchConfig: DeserializeOwned {
	...
	bandcamp: Bandcamp<SearchConfig>,
}
...
#[derive(Debug, Deserialize)]
#[serde(rename_all(deserialize = "kebab-case"))]
struct Bandcamp<SearchConfig> where SearchConfig: DeserializeOwned {
	sim_threshold: Sim,
