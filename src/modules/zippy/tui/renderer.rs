...
impl<'a> Renderer<'a> {
	fn render_title(&self) -> (Cow<'a, str>, Style) {
		match &self.0.status {
			...
			Status::NoEntries => (
				" Zippy - no entries found ".into(),
				Style::default()
			),

			Status::Error(error) => (
				format!(" Zippy - {} ", error).into(),
				Style
					::default()
					.fg(Color::Red)
			),
					...
					let (label, style) = match status {
						...
						ItemStatus::Filtered(filter) => {
							let text = match filter {
								...
								Filter::Blacklist(items) => format!(
									"{} | id blacklisted by regexes: {:?}",
									label,
									items
								),
